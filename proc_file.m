function [ bl, blu ] = proc_results(kk, type, stans)
bl=[];
STYPE=3;
for ECON=10:10:10
    for MWTYPE=2:2
        ...
        for CTYPE=2:2
           for LSTYPE=1:1
            %for FORSIGHT=1:1:10
               ...
               filename2=strcat('C:\Users\AU289559\Desktop\abacus  sim\partialrez\varikeanew\ikeavar%5C22131010k6'); 
               filename=strcat(filename, int2str(CTYPE));
                filename=strcat(filename, int2str(LSTYPE));
                filename=strcat(filename, int2str(STYPE));
                filename=strcat(filename, int2str(FORSIGHT));
              ...
              %  bi=load (filename2, 'ans');
                bi=stans;
                rez=process_z(bi.ans, benchmark6lf1.ans, 100);
                i=size(bl,1)+1;
                    bl(i,1)=ECON;
                    bl(i,2)=MWTYPE;
                    bl(i,3)=CTYPE;
